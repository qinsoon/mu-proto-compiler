...
public class burgListenerImpl extends burgBaseListener {
    ...
    @Override public void exitMcCallExpDecl(@NotNull burgParser.McCallExpDeclContext ctx) {
    	for (burgParser.IdStringContext s : ctx.idString())
    		Burg.MC_CALL_EXP.add(s.getText());
    }
    
    @Override public void exitMcTailCallExpDecl(@NotNull burgParser.McTailCallExpDeclContext ctx) {
    	for (burgParser.IdStringContext s : ctx.idString())
    		Burg.MC_TAILCALL.add(s.getText());
    }
...
//        format = format.replaceAll("COMMA", ",");
    ...
    private CCTOperand getOperand(McOperandContext operandCtx) {
        ...
        if (operandCtx instanceof burgParser.McOpdNodeChildContext) {
        	...
        	CCTOperand disp  = getOperand(((burgParser.McOpdAddressContext) operandCtx).mcOperand(1));
        	CCTOperand index = getOperand(((burgParser.McOpdAddressContext) operandCtx).mcOperand(2));
        	CCTOperand scale = getOperand(((burgParser.McOpdAddressContext) operandCtx).mcOperand(3));
        	
        	ret = new OpdMemOperand(base, disp, index, scale);
        }
